...
class AdjustmentGeneralViewController: BaseViewController,UITextViewDelegate,SingleSelectDropdownDelegate,ConfirmationViewDelegate,AddAttachmentViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if adjustmentType == "QUARANTINE" || adjustmentType == "DISPENSING" || adjustmentType == "DESTRUCTION" || adjustmentType == "TRANSFER" || adjustmentType == "MISC_ADJUSTMENT"{
    ...
    func refreshProductView(){
        ...
        productCount = productCount + serialCount
        let itemCount = itemsList!.count
        var productStr = ""
        if productCount>1 {
            productStr = "Items"
        }else{
        ...
        countTextFiled.text = "\(productCount)" + " \(productStr) , " + "\(itemCount)" + " \(itemstr)."
    }
    ...
    func formValidation()->Bool{
        ...
        if let dataDict = Utility.getDictFromdefaults(key: "adjustment_general_info") {
            ...
            if location.isEmpty {
                ...
                isValidated = false
            }else if adjustmentType == Adjustments_Types.Transfer.rawValue {
...
//                }
    ...
    @IBAction func nextButtonPressed(_ sender: UIButton) {
        ...
        defaults.set(true, forKey: "adjustment_1stStep")
...
extension AdjustmentGeneralViewController: UITableViewDelegate, UITableViewDataSource {
...
extension AdjustmentGeneralViewController:SingleScanViewControllerDelegate{

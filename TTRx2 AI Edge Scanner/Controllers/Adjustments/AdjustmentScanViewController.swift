//
//  QuarantineScanViewController.swift
...
class AdjustmentScanViewController: BaseViewController {
    ...
    var productCount = 0
    var serialCount = 0
    var isAdjustmentLotAdded : Bool = false
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if isAdjustmentLotAdded && triggerScanEnable{
            isAdjustmentLotAdded = false
                        ...
                        if responseArray.count > 0{
                            if let serialDetailsArray = responseArray as? [[String : Any]]{
                                ...
                                if !self.verifiedSerials.isEmpty{
                                        ...
                                        if let arr = Utility.getObjectFromDefauls(key: "AdjustmentVerifiedArray"){
                                        ...
                                        Utility.saveObjectTodefaults(key: "AdjustmentVerifiedArray", dataObject: defaultData)
                                }
                            }
                        }else{
    ...
    func moveToLotBasedView(){
        let controller = self.storyboard?.instantiateViewController(withIdentifier: "AdjustmentLotBasedView") as! AdjustmentLotBasedViewController
    ...
    @IBAction func nextButtonPressed(_ sender: UIButton) {
        ...
        if(productCount == 0 && serialCount == 0){
            Utility.showPopup(Title: App_Title, Message: "Please scan serials or Add Lot based items before proceed.".localized(), InViewC: self)
            return
        }
        
        defaults.set(true, forKey: "adjustment_2ndStep")
    ...
    @IBAction func viewScannedSerialsButtonPressed(_ sender: UIButton) {
        let controller = self.storyboard?.instantiateViewController(withIdentifier: "AdjustmentScanedSerialsView") as! AdjustmentScanedSerialsViewController
        self.navigationController?.pushViewController(controller, animated: true)
    ...
    @IBAction func failedItemsButtonButtonPressed(_ sender:UIButton){
        self.failedItemsDetails()
...
extension AdjustmentScanViewController : ScanViewControllerDelegate{
        ...
        if failedArr.count > 0 {
            ...
            for faileditems in failedArr {
                if !(failedSerials as NSArray).contains(faileditems){
                    ...
                    Utility.saveObjectTodefaults(key: "AdjustmentVerifiedArray", dataObject: self.failedSerials)
                    self.checkNewfailedItems()
                }
            }
        }
...
extension AdjustmentScanViewController : SingleScanViewControllerDelegate{
...
extension AdjustmentScanViewController : AdjustmentLotBasedDelegate {
    func didAddedLotBasedProduct() {
        isAdjustmentLotAdded = true
    }
}
extension AdjustmentScanViewController : FailedItemsViewDelegate{
     func failedProductDetails(itemArr : Array<Any>){
        Utility.saveObjectTodefaults(key: "ScanFailedItemsArray", dataObject: itemArr)
        Utility.saveObjectTodefaults(key: "AdjustmentVerifiedArray", dataObject: itemArr)
    }
}

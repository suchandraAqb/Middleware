//
//  AdjustmentLotBasedViewController.swift
...
import CoreData
import AVFoundation
@objc protocol AdjustmentLotBasedDelegate : class {
    @objc optional func didAddedLotBasedProduct()
}
class AdjustmentLotBasedViewController: BaseViewController,AdjustmentAddLotBasedDelegate,UITableViewDataSource, UITableViewDelegate,ProductLotStorageDelegate {
    weak var delegate : AdjustmentLotBasedDelegate?
    ...
    override func viewDidLoad() {
        ...
        if isFromScan ?? false {
            scanDisclaimerLabel.text = "Some serials did not match exactly but quantity could be found with their Product/Lot combination.".localized() + "\n" + "Those items are scanned as Lot based".localized()
        }else{
    ...
    func fetchItemsWithNoStorage() -> [NSManagedObject]{
        ...
        do{
            ...
            return_obj = try PersistenceService.context.fetch(Adjustments.fetchRequestWithPredicate(predicate: predicate))
    ...
    @IBAction func quantityUpdateButtonPressed(_ sender: UIButton) {
        ...
        if let dict = self.itemsList![sender.tag] as? NSDictionary {
            ...
            if let qty = dict["lot_max_quantity"] as? Int {
                maxQuantity = qty
            }
    ...
    @IBAction func selectStorageButtonPressed(_ sender: UIButton) {
        ...
        if let dict = self.itemsList![sender.tag] as? NSDictionary {
            ...
            controller.isFromInventory = isFromInventory
    ...
    @IBAction func addProductButtonPressed(_ sender: UIButton) {
        let controller = self.storyboard?.instantiateViewController(withIdentifier: "AdjustmentAddLotBasedView") as! AdjustmentAddLotBasedViewController
        controller.delegate = self
            ...
            if let filterArray = allList?.filtered(using: predicate){
                if filterArray.count > 0 {
                    ...
                    if let dataDict = filterArray.first as? NSDictionary {
                        ...
                        if let name = dataDict["gtin"] as? String{
                            headerView.gtinLabel.text = name
                        }
                        
                        if let name = dataDict["identifier_value"] as? String{
                            headerView.ndcLabel.text = name
                        }

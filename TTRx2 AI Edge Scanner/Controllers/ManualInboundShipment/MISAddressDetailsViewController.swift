//
//  MISAddressDetailsViewController.swift
...
class MISAddressDetailsViewController: BaseViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    func setup_view(){
        ...
        if isSeller{
            ...
            backButton.setTitle("Buyer Address".localized(), for: .normal)
            toggleButton1.setTitle("Bought By".localized(), for: .normal)
...
//            }
          ...
          if !toggleButton1.isUserInteractionEnabled{
              ...
              if view1SelectedIndex > -1{
                  if let data =  addressArr![view1SelectedIndex] as? NSDictionary {
                      Utility.saveDictTodefaults(key: "MIS_soldBy", dataDict: data)
                  }
              }
              ...
              if view2SelectedIndex > -1{
                  if let data =  addressArr![view2SelectedIndex] as? NSDictionary {
                      Utility.saveDictTodefaults(key: "MIS_shipFrom", dataDict: data)
                  }
              }
            ...
            if !toggleButton1.isUserInteractionEnabled{
               ...
               if view1SelectedIndex > -1{
                   if let data =  addressArr![view1SelectedIndex] as? NSDictionary {
                       Utility.saveDictTodefaults(key: "MIS_broughtBy", dataDict: data)
                   }
               }
               ...
               if view2SelectedIndex > -1{
                   if let data =  addressArr![view2SelectedIndex] as? NSDictionary {
                       Utility.saveDictTodefaults(key: "MIS_shipTo", dataDict: data)
                   }
               }
    ...
    @IBAction func addCustomAddressButtonPressed(_ sender: UIButton) {
        ...
        if isSeller{
          ...
          if !toggleButton1.isUserInteractionEnabled{
              addresstype = "MIS_soldBy"
          }else{
              addresstype = "MIS_shipFrom"
          }
        }else{
            
            if !toggleButton1.isUserInteractionEnabled{
                addresstype = "MIS_broughtBy"
            }else{
                addresstype = "MIS_shipTo"
           }
        ...
        let controller = self.storyboard?.instantiateViewController(withIdentifier: "MISAddCustomAddressView") as! MISAddCustomAddressViewController
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "MISCustomerAdressListCell") as! MISCustomerAdressListCell
...
class MISCustomerAdressListCell: UITableViewCell
{

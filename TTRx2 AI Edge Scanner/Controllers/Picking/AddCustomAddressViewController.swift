//
//  AddCustomAddressViewController.swift
//  TTRx2 AI Edge Scanner
//
//  Created by Amit Yadav on 15/05/20.
//  Copyright © 2020 AQB Solutions Private Limited. All rights reserved.
//

import UIKit

class AddCustomAddressViewController: BaseViewController,SingleSelectDropdownDelegate {
    ...
    func formValidation()-> Bool{
        ...
        if let rNameStr = rNameTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines), rNameStr.isEmpty {
            Utility.showPopup(Title: App_Title, Message: "Please enter recipient name.".localized(), InViewC: self) //,,,sb-lang2
            isValidated = false
               
        }else if let address = addressTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines), address.isEmpty {
            Utility.showPopup(Title: App_Title, Message: "Please enter address.".localized(), InViewC: self) //,,,sb-lang2
            isValidated = false
               
        }else if let city = cityTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines), city.isEmpty {
            Utility.showPopup(Title: App_Title, Message: "Please enter city name.".localized(), InViewC: self) //,,,sb-lang2
            isValidated = false
               
        }else if isStateRequired ?? false && stateName.isEmpty{
            Utility.showPopup(Title: App_Title, Message: "Please select state".localized(), InViewC: self) //,,,sb-lang2
            isValidated = false
        }
        else if postalCode.isEmpty {
            Utility.showPopup(Title: App_Title, Message: "Please enter postal code.".localized(), InViewC: self) //,,,sb-lang2
            isValidated = false
        ...
        /*}else if !(postalRegex ?? "").isEmpty && !postalCode.validatedStringWithRegex(regexStr: postalRegex ?? ""){
            ...
            Utility.showPopup(Title: App_Title, Message: "Please enter valid Email.".localized(), InViewC: self) //,,,sb-lang2
            isValidated = false
        }
              ...
              DispatchQueue.main.async{
                  ...
                  if isDone! {
                        ...
                        if let response = responseData as? Array<Any>{
                            ...
                            if let firstData = response.first as? NSDictionary {
                                let btn = UIButton()
                                btn.tag = 1
    ...
    @IBAction func doneButtonPressed(_ sender: UIButton) {
        ...
        if formValidation(){
            ...
            for  controller in controllers {
                ...
                if controller.isKind(of: PickingConfirmationViewController.self){

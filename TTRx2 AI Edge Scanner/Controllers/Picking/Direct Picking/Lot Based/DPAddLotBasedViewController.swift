...
@objc protocol  DPAddLotBasedDelegate: class {
    @objc optional func didProductAdded()
}

class DPAddLotBasedViewController:BaseViewController,UITableViewDataSource, UITableViewDelegate,SingleSelectDropdownDelegate,DPProductLotStorageDelegate
{
    
    weak var delegate: DPAddLotBasedDelegate?
    ...
    @IBAction func selectLotButtonPressed(_ sender: UIButton) {
        if lots == nil || lots?.count == 0 {
            ...
            lotlistTable.isHidden = true
            Utility.showPopupWithAction(Title: "", Message: "Picking will be done with different lot than what was suggested.", InViewC:self, action: {
                self.lotlistTable.isHidden = false
    ...
    func populateSelectedProductView(){
        ...
        lots = nil
        if let uuid = selectedProductDict!["product_uuid"] as? String{
...
//        }
    ...
    func getProductsList(){
        let loc = defaults.object(forKey: "SOPickingLocationUUID") as! String
        let appendStr = "lots/?sort_by=lot_expiration&sort_by_asc=true&lot_type=LOT_BASED&location_uuid=\(loc)&is_in_stock_only=true"
              ...
              DispatchQueue.main.async{
                  ...
                  if isDone! {
                        ...
                        if let dataArray = responseDict["data"] as? Array<Any> {
                            ...
                            let defaultsSOProduct = defaults.object(forKey: "SOProductUuid") as? NSArray
                            for productDict in dataArray {
                                let productDetailsDict = productDict as? NSDictionary
                                let uuid = productDetailsDict!["product_uuid"] as? String
                                if defaultsSOProduct!.contains(uuid ?? ""){
                                    filterArr.add(productDict)
                                }
                            }
                            if !(filterArr.count>0) {
                                Utility.showPopup(Title: "", Message: "Picked Items not available for lot based.".localized(), InViewC: self)
                            }
                            self.products = filterArr as? Array<Any>
                            self.itemsList = filterArr as? Array<Any>
    ...
    func getProductsLotList(uuid:String){
        ...
        let appendStr = "\(uuid)/lots?location_uuid=\(location_uuid)&sort_by=expiration_date"
        self.showSpinner(onView: self.view)
          Utility.GETServiceCall(type: "GetProducts", serviceParam: {}, parentViewC: self, willShowLoader: false,viewController: self, appendStr: appendStr) { (responseData:Any?, isDone:Bool?, message:String?) in
              DispatchQueue.main.async{
                  self.removeSpinner()
                  if isDone! {
                        ...
                        if let dataArray = responseData as? NSArray {
                            ...
                            if filterArr.count>0{
                                ...
                                self.selectedLot = self.lots?.first as! [String : Any]
                                self.selecteditem(data: self.selectedLot as NSDictionary, sender: UIButton())
                            }else{
                                self.lots = nil
                                self.masterLots = nil
                                self.lotNameLabel.text = ""
                            }
    ...
    func addProduct(quantity:String,lot:String,maxQuantity:String){
        ...
        obj.product_name = (selectedProductDict?["product_name"] as? String) ?? ""
        obj.product_uuid = (selectedProductDict?["product_uuid"] as? String) ?? ""
        obj.lot_no = lot
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == listTable {
           ...
           view.endEditing(true)
            ...
            if let uuid = product?["product_uuid"] as? String{

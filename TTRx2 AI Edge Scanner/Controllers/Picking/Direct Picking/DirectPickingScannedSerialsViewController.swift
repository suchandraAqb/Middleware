//
//  DirectPickingScannedSerialsViewController.swift
...
class DirectPickingScannedSerialsViewController: BaseViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    func removeSerialFromDB(obj:DirectPicking){
    ...
    func prepareSerialsForRemove(barcodes:[String]){
        ...
        for barcode in barcodes{
            ...
            do{
                ...
                let adjustment_obj = try PersistenceService.context.fetch(DirectPicking.fetchRequestWithPredicate(predicate: predicate))
                ...
                if !adjustment_obj.isEmpty{
                    ...
                    for obj in adjustment_obj{
                        ...
                        let index = (serialList as NSArray).index(of: obj)
        ...
        DispatchQueue.main.async{
            self.serialListTable.reloadData()
            if Int(sCount)>0 {
                Utility.showPopup(Title: Success_Title, Message: "\(Int(sCount)>1 ?"\(sCount) Serials" : "\(sCount) Serial")" + " successfully removed".localized()  , InViewC: self)
            }//+ "\n\(Int(fCount)>1 ?"\(fCount) Serials" : "\(fCount) Serial")" + "failed to removed".localized()
        }
...
extension DirectPickingScannedSerialsViewController : ScanViewControllerDelegate{
...
extension DirectPickingScannedSerialsViewController : SingleScanViewControllerDelegate{
...
extension DirectPickingScannedSerialsViewController : ConfirmationViewDelegate{

//
//  ARSFCategoryCriteriaViewController.swift
...
class ARSFCategoryCriteriaViewController: BaseViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    @IBOutlet var categoryListTableHeightConstraint: NSLayoutConstraint!
...
//    var selectedCategoryArray = [String]()
    ...
    override func viewDidLoad() {
        ...
        if selectedCategoryArray.count > 1 {
            let name = selectedCategoryArray[1] as! String
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        categoryListTable.addObserver(self, forKeyPath: "contentSize", options: NSKeyValueObservingOptions.new, context: nil)
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        matchTypeListTable.removeObserver(self, forKeyPath: "contentSize")
        categoryListTable.removeObserver(self, forKeyPath: "contentSize")
    }
    //MARK:- End
    ...
    @IBAction func doneButtonPressed(_ sender: Any) {
        ...
        if selectedCategoryArray.count == 0 {
            isvalidate = false
            Utility.showPopup(Title: App_Title, Message: "Please select category.".localized(), InViewC: self)
            return
        }
        ...
        if isvalidate {
            ...
            valueToMatchArray.append(selectedCategoryArray)
            //,,,sb11-10
            
//            let valueToMatchArrayJson = Utility.json(from: selectedCategoryArray)
            ...
            if isAdd {
                ...
                obj.critera_type = "CategoryBased"
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        categoryListTableHeightConstraint.constant=categoryListTable.contentSize.height
                ...
                if isDone! {
                    if let response = responseData as? NSDictionary {
                        if let dataArr = response["data"] as? Array<Any>{
                            self.categoryListArray = dataArr
//                            print ("dataArr....<<<>>>",dataArr) //,,,sb11-12
...
//                         print("self.matchTypeListArray...>>>",operand_type,self.matchTypeListArray as Any,self.matchTypeListArray!.count) //,,,sb11-12
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        }else if tableView == categoryListTable{
            return categoryListArray?.count ?? 0//,,,sb2
        }else{
            return 0
        }
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == matchTypeListTable {
            let cell = tableView.dequeueReusableCell(withIdentifier: "ARSFCategoryCriteriaMatchTypeListCell") as! ARSFCategoryCriteriaMatchTypeListCell
...
//                    cell.titleLabel.textColor = UIColor.blue
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "ARSFCategoryCriteriaCategoryListCell") as! ARSFCategoryCriteriaCategoryListCell
            
            //,,,sb2
            let dict = self.categoryListArray![indexPath.row] as? NSDictionary
            var dataStr = ""
            ...
            //,,,sb11-1
            /*
             if let categoryDict = self.categoryListArray![indexPath.row] as? NSDictionary {
                print("\(categoryDict)")
                if selectedCategoryArray.contains(dataStr) {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
        if tableView == matchTypeListTable{
            let cell = tableView.cellForRow(at: indexPath) as! ARSFCategoryCriteriaMatchTypeListCell
...
//            let cell = tableView.cellForRow(at: indexPath) as! ARSFCategoryCriteriaCategoryListCell
...
//                print("\(categoryDict)") //,,,sb11-12
                /*
                if selectedCategoryArray.contains(categoryDict) {
                    /*if let index = selectedCategoryArray.firstIndex(of: categoryDict) {
                        selectedCategoryArray.remove(at: index)
                    }*///,,,sb11-9
                }
                else {
                    selectedCategoryArray.removeAll()//,,,sb11-9
                    selectedCategoryArray.append(categoryDict)
                }*/
                
                /*if let txt = categoryDict["name"] as? String,!txt.isEmpty {
                    if selectedCategoryArray.contains(txt) {
                        /*if let index = selectedCategoryArray.firstIndex(of: txt) {
                            selectedCategoryArray.remove(at: index)
                        }*///,,,sb11-9
                    }
                    else {
                        selectedCategoryArray.removeAll()//,,,sb11-9
                        selectedCategoryArray.append(txt)
                    }
                }*/
                
                //,,,sb11-10
                if let idInt = categoryDict["id"] as? Int {
                    var name = ""
                    if let txt = categoryDict["name"] as? String,!txt.isEmpty {
                        name = txt
                    }
                    ...
                    selectedCategoryArray.append(idInt)
                    selectedCategoryArray.append(name)
            ...
            self.categoryListTable.reloadData()//,,,sb11-9
            //,,,sb11-1
        }
    }
    //MARK:- End
...
class ARSFCategoryCriteriaMatchTypeListCell:UITableViewCell{
...
class ARSFCategoryCriteriaCategoryListCell:UITableViewCell{
    @IBOutlet var multiLingualViews: [UIView]!

...
//        self.layoutIfNeeded()
    ...
    func prepareView(barCode : TrackedBarcode) -> CustomView{
...
//    func loadView(barCode : TrackedBarcode) -> CustomView{
//        let customInfoWindow = Bundle.main.loadNibNamed("CustomView", owner: self, options: nil)?[0] as! CustomView
...
//    }
    ...
    @IBAction func proceed(_ sender: UIButton) {
       self.delegate?.didTappedOnProceed(trackedBarcode: trackedBarcode)
    }
...
//                }
//            }
            self.delegate?.didTappedOnSummaryView(customView: self)

        }else{
            self.delegate?.didGetError(customView: self)
        }
    }
}
